...
import userListRouter from '../Routes/users1-list'; //users-list routes
import bookListRouter from '../Routes/book-list';
...
db.on("open", function()
{
  console.log(`Connected to MongoDB at: ${(DBConfig.RemoteURI) ? DBConfig.HostName : "Local Host"}`);
});
...
app.use('/api',passport.authenticate('jwt',{session:false}),userListRouter);
app.use('/api',passport.authenticate('jwt',{session:false}),bookListRouter);
